...
#could optimize with count here
...
resource "aws_security_group" "wp_dev_sg" {
  ...
  ingress {
    ...
    to_port = 80
    protocol = "tcp"
    cidr_blocks = ["${var.localip}"] #change to ["0.0.0.0/0"] when going live, otherwise, only you ip will be able to hit it
  }
...
#--------- ELB -----------  This is old, do App or network LB in future

resource "aws_elb" "wp_elb" {
  name = "${var.doman_name}-elb"
...
resource "aws_ami_from_instance" "wp_golden" {
  name = "wp_ami-${random_id.golden_ami.golden_ami.b64}"
...
resource "aws_launch_configuration" "wp_lc" {
  name_prefix = "wb_lc-"
  image_id = "${aws_ami_from_instance.wp_golden.wp_golden.id}"
  ...
  # for blue green deployments, will create the new launch config before destroying the old one
  lifecycle {
...
resource "aws_route53_record" "www" {
  ...
  alias {
    ...
    #this is where you could set a static fallback for your website
